...
public class SpendRepositoryHibernate implements SpendRepository {
    ...
    public Optional<CategoryEntity> findCategoryById(UUID id) {
        try {
            return Optional.of(
                    entityManager.createQuery("select u from CategoryEntity u where u.id =: id", CategoryEntity.class)
                            .setParameter("id", id)
                            .getSingleResult()
            );
    ...
    public Optional<SpendEntity> findById(UUID id) {
        try {
            return Optional.of(
                    entityManager.createQuery("select s from SpendEntity s where s.id =: id", SpendEntity.class)
                            .setParameter("id", id)
                            .getSingleResult()
            );
        } catch (NoResultException e) {
