package guru.qa.niffler.extractor;
            ...
            if (requesterId != null) {
                ...
                if (user != null) {
                    if (user.getId().equals(requesterId)) {
                        if (user.getFriendshipRequests().stream().noneMatch(friendshipRequest ->
                                friendshipRequest.getRequester().getId().equals(requesterId) &&
                                        friendshipRequest.getAddressee().getId().equals(addresseeId))) {
                            user.getFriendshipRequests().add(friendship);
                        }
                    } else if (user.getId().equals(addresseeId)) {
                        if (user.getFriendshipAddressees().stream().noneMatch(friendshipRequest ->
                                friendshipRequest.getRequester().getId().equals(requesterId) &&
                                        friendshipRequest.getAddressee().getId().equals(addresseeId))) {
